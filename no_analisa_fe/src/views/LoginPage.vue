...
const handleLogin = async () => {
    ...
    if (!credentials.value.username) {
        triggerToast('Username wajib diisi', 'error')
        return;
    }

    if (!credentials.value.password) {
        triggerToast('Password wajib diisi', 'error')
        return;
    }
  ...
  try {
    ...
    if (response.success) {
      ...
      localStorage.setItem('userAccount', JSON.stringify(response.data.userAccount));
      
      router.push('/'); 
    } else {
        triggerToast(response.error || 'Ada kesalahan pada username atau password anda, periksa kembali!', 'error');
    }
  } catch (err) {

    const errorMap = {
        'Invalid credentials': 'Ada kesalahan pada username atau password anda, periksa kembali',
        'User not found': 'Account anda tidak ada',
        'Failed to fetch': 'Tidak dapat konek server. Cek jaringan.'
    };
    errorMessage.value = errorMap[err.message] || err.message || 'An error occurred during login';
...
<template>
  <div class="login-container">
        <div class="login-card">
            ...
            <form autocomplete="off" @submit.prevent="handleLogin" class="login-form" id="loginForm" novalidate>
                <div class="form-group">
                    <div class="input-wrapper">
                        <input v-model="credentials.username" 
                            type="text" 
                            class="input input-bordered w-full bg-base-200 border-base-300 text-base-content focus:input-primary focus:outline-none" 
